	...
	<tr>
		...
		<td width="100%" class="ver01" >
			<div class="box2" panelTitle="查询" showStatus="false">
				<form action="/confession!getconfessionOfPager.action" id="queryForm" method="post">
				<table>
					<tr>
						<td>忏悔类型：</td>
						<td>
...
<script type="text/javascript">
	...
	function initGrid() { 
		grid = $("#dataBasic").quiGrid({
		 ...
		 url: '/confession!getconfessionOfPager.action?confessionJson.returnuid='+'${loginedUser.id}', sortName: 'id',rownumbers:true,checkbox:true,
	...
	function addUnit() {
		top.Dialog.open({
				URL:"/clxjmain/clxjmain-management-content.jsp",
				Title:"添加",Width:800,Height:550}); 
	}
	//查看
	function onView(rowid){
		top.Dialog.open({
			URL:"/confession!getconfessionOfId.action?confession.id=" + rowid,
			Title:"详情",Width:800,Height:550});
	}
	//修改	
	function onEdit(rowid){
		top.Dialog.open({
			URL:"/clxjmain!getclxjmainOfId.action?isupdate=1&clxjmain.id=" + rowid,
	...
	function onDelete(rowid,rowidx){
		top.Dialog.confirm("确定要删除该记录吗？",function(){
		  	//删除记录
		  	$.post("/clxjmain!deleteClxjmain.action",
		  			{"ids":rowid},
		  			function(result){
